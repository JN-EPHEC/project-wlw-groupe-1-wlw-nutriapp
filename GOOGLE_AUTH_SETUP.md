...
5. Dans **Authorized redirect URIs**, ajoutez :
   - `https://auth.expo.io/@votre-username/Project-BA` (pour Expo Go via le proxy Expo)

> Important: pour un client OAuth de type **Web application**, Google n'accepte que des redirect URIs `http(s)://...`.
> N'ajoutez pas `nutriadapt://oauth` à un client Web.

Pour une app native *standalone* (sans proxy Expo), utilisez des client IDs spécifiques **Android** / **iOS** et configurez `EXPO_PUBLIC_GOOGLE_ANDROID_CLIENT_ID` / `EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID`.
